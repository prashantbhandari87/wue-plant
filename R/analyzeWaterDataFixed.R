...
analyzeWaterDataFixed <- function(water_data, predicted_pcv_data, project_code) {
  ...
  water_df <- water_df %>% filter(weight.before > -1) %>%
    ...
    group_by(plantbarcode) %>%
    mutate(time.water.loss = as_datetime(timestamp) - as_datetime(lag(timestamp, 1))) %>%
    group_by(plantbarcode, day) %>%
    arrange(timestamp) %>%
    mutate(watering.job.num = 1:n()) %>%
    arrange(plantbarcode, day, watering.job.num) %>%
    group_by(plantbarcode, day) %>%
    summarize(water.amount.plus.dap = sum(water.amount.plus),
              .groups = "drop")
  mutate(water.amount.plus.dap = sum(water.amount.plus))

  water.join <- water_df %>%
    select(plantbarcode, genotype, treatment, replicate,
           water.amount, water.amount.plus, day,water.amount.plus.dap)
