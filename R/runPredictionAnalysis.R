...
runPredictionAnalysis<-function (data) {
  ...
  genotype_subset <- function(geno, data) {
    ...
    loess_fit <- loess(area.pixels ~ DAP, data = subset_data,
                       span = 0.57)
  ...
  predictions.30 <- lapply(genotypes, function(geno) {
    tryCatch({
      ...
      problematic_genotypes <<- c(problematic_genotypes,
                                  geno)
      NULL
    })
  })
  ...
  predictions.30 <- predictions.30 %>% filter(!genotype %in%
                                                problematic_genotypes)
  predictions.100 <- predictions.100 %>% filter(!genotype %in%
                                                  problematic_genotypes)
  dda.30 <- predictions.30 %>% group_by(genotype, DAP) %>%
    select(genotype, DAP, predicted_area_pixels) %>% distinct() %>%
    group_by(genotype) %>% arrange(genotype, DAP) %>% mutate(daily_area_diff_pred = predicted_area_pixels -
                                                               lag(predicted_area_pixels, default = 0)) %>% mutate(daily_area_diff_pred = if_else(daily_area_diff_pred <
                                                                                                                                                    0, NA_real_, daily_area_diff_pred)) %>% mutate(out = predicted_area_pixels -
                                                                                                                                                                                                     lag(predicted_area_pixels, default = 0)) %>% mutate(daily_area_diff_pred = if_else(out <
                                                                                                                                                                                                                                                                                          0, NA_real_, daily_area_diff_pred)) %>% select(-c(out))
  predictions.30 <- predictions.30 %>% select(-c(predicted_area_pixels))
  predictions.30 <- left_join(predictions.30, dda.30, by = c("genotype","DAP"))

  if (predictions.30$daily_area_diff_pred %>% is.na() %>% sum() ==
      0) {
    print("Areas not shrinking")
  }else {
    cat("The number of cases with shrinking areas is:\n")
    print(predictions.30$daily_area_diff_pred %>% is.na() %>%
            sum())
  }

  dda.100 <- predictions.100 %>% group_by(genotype, DAP) %>%
    select(genotype, DAP, predicted_area_pixels) %>% distinct() %>%
    group_by(genotype) %>% arrange(genotype, DAP) %>% mutate(daily_area_diff_pred = predicted_area_pixels -
                                                               lag(predicted_area_pixels, default = 0)) %>% mutate(daily_area_diff_pred = if_else(daily_area_diff_pred <
                                                                                                                                                    0, NA_real_, daily_area_diff_pred)) %>% mutate(out = predicted_area_pixels -
  ...
  predictions.100 <- predictions.100 %>% select(-c(predicted_area_pixels))
  predictions.100 <- left_join(predictions.100, dda.100, by = c("genotype","DAP"))

  if (predictions.100$daily_area_diff_pred %>% is.na() %>%
      sum() == 0) {
    print("Areas not shrinking")
  }else {
    cat("The number of cases with shrinking areas is:\n")
    print(predictions.100$daily_area_diff_pred %>% is.na() %>%
            sum())
  }
