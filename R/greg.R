...
###################
transformationTableFunction <- function(dat1) {
  transformationTable <- data.frame(
    "trait" = character(0),
    "lambda" = numeric(0),
    "maxLambda" = numeric(0),
    "low95CI" = numeric(0),
  ...
  dataToTransform <- dat1
  transformAllData <- dat1
  transformAllData[, 1:9] <- NA
  transformStandAllData <- transformAllData
  
  for (i in 1:dim(dat1)[2]) {
    if (all(is.na(dataToTransform[, i]))) {
      next()
    }
    dataToTransform[, i] <- dataToTransform[, i] + abs(min(dataToTransform[, i], na.rm = TRUE)) + 1
    
    b <- MASS::boxcox(dataToTransform[, i] ~ 1, plotit = FALSE, lambda = seq(-5, 5, len = 201))
    lim <- max(b$y) - qchisq(19/20, 1) / 2
    lowCI <- min(b$x[b$y >= lim])
    highCI <- max(b$x[b$y >= lim])
    
    if (1 <= highCI && 1 >= lowCI) {
      transformAllData[, i] <- dataToTransform[, i]
      lambda <- 1
    } else if (0 <= highCI && 0 >= lowCI) {
      transformAllData[, i] <- log(dataToTransform[, i])
      lambda <- 0
    } else if (0.5 <= highCI && 0.5 >= lowCI) {
      transformAllData[, i] <- (dataToTransform[, i]^0.5 - 1) / 0.5
      lambda <- 0.5
    } else if (-0.5 <= highCI && -0.5 >= lowCI) {
      transformAllData[, i] <- (dataToTransform[, i]^-0.5 - 1) / -0.5
      lambda <- -0.5
    } else if (-3 <= highCI && -3 >= lowCI) {
      transformAllData[, i] <- (dataToTransform[, i]^-3 - 1) / -3
      lambda <- -3
    } else if (-1 <= highCI && -1 >= lowCI) {
      transformAllData[, i] <- (dataToTransform[, i]^-1 - 1) / -1
      lambda <- -1
    } else {
      transformAllData[, i] <- (dataToTransform[, i]^b$x[which.max(b$y)] - 1) / b$x[which.max(b$y)]
      lambda <- b$x[which.max(b$y)]
    }
    transformStandAllData[, i] <- standardize(transformAllData[, i])
  }
  
  
  
  return(transformAllData)
}
